...
auto aggregation_from_json(nlohmann::json &j)
    ...
    if (type == "interval") {
        ...
        return std::make_unique<IntervalAggregationStrategy>(keep_amount, name,
                                                             interval);
    } else if (type == "readings") {
        auto nreadings = j.at("readings").get<uint32_t>();
        return std::make_unique<ReadingsAggregationStrategy>(keep_amount, name,
                                                             nreadings);
    } else {
